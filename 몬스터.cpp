	...
	while (1)
	{
		...
		if (menuCode == 0)
		{
			...
			while (playing)
			{
				...
				for (i = 0; i < 20; i++)
				{
					_beginthreadex(NULL, 0, mm, (void*)&list_MON[i], 0, NULL);
				}
				while (playing)
				{
					mKey = keyControl();
...
void monster_move(int mappos, int posx, int posy, int x ,int y)
{
...
unsigned _stdcall mm(void*p)
{
	srand(rand());
	int x, y;
	monster* mp = (monster*)p;
	monster m = *mp;
	while (1)
	{
		if (end)
		...
		x = rand() % 3 - 1;
		y = rand() % 3 - 1;
		monster_move(m.mappos, m.posx, m.posy, x, y);
		m.posx += x;
		m.posy += y;
		Sleep(1000);
	}
